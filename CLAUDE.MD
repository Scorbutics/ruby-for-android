...
1. **Configuration Phase** (`./scripts/configure.sh`)
   ...
   - Reads toolchain params from `arm64-v8a-android-toolchain.params` by default but any other toolchain-like file can be provided.
...
1. (Optional) If targeting Android, install Android NDK r23b+
2. Install Ruby 3.0.0+ on host (needed by `make install` step)
3. Install CMake 3.10+
4. Edit `arm64-v8a-android-toolchain.params` with NDK path or create your own one for other platforms
5. Run `./configure`
